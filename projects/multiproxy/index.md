...
- 主役は **オンプレ相当検証環境**（WSL2 + Docker Compose を中心に、VMware上のSamba AD/DC・dnsmasqと連携 ／ **実装・検証済み**）
- AWS 関連ページは **責務分離を写像する設計案**（**設計のみ・実装未実施**）として位置づけています

---

## 📌 はじめて見る方向け：このページの読み方

ページが長いため、**目的に合わせた時間配分**を目安にしてください。

| 想定読者の状況 | 目安 | 推奨ルート |
|---|---|---|
| 軽く内容を把握したい | **約3分** | [このポートフォリオで伝えたいこと](#このポートフォリオで伝えたいこと) と [✅ 動作検証（証跡）](#-動作検証スクリーンショット集) の見出しだけ確認 |
| ある程度時間がある | **約10分** | 上記 + [2. システムアーキテクチャ](#2-システムアーキテクチャ全体像) / [Verification](./docs/verification.html) の要点 |
| 詳細までじっくり見たい | **約30分** | 上記 + [4. 設計補足](#4-設計補足通信制御に関する考慮点) / [5. 苦労した点](#5-苦労した点と解決アプローチ) / [Automation](./docs/automation.html) / [10分詳細版PPTX](./Mulutiproxy_System_AWS_10min_ver3.pptx) |
| 設計判断まで深掘りしたい | **追加 約20分** | [AWS 移行設計案](./docs/aws-deployment-plan.html)（実装未実施） |
| 口頭説明用 | — | [ポートフォリオ説明用スクリプト](./docs/interview-pitch.html)（5分概要版・10分詳細版に対応） |

### 📌 発表資料（PPTX）

本ポートフォリオは、Markdown による詳細説明に加えて、短時間で全体像を把握するための発表資料も用意しています。

- **5分概要版** — オンプレ相当検証環境の全体像、代表的なログ切り分け、再現性を確認する資料: [`Mulutiproxy_System_AWS_5min_ver2.pptx`](./Mulutiproxy_System_AWS_5min_ver2.pptx)
- **10分詳細版** — 3段Proxy、認証・DNS・PAC、ログ・監視、AWS移行設計案まで含めた詳細説明資料: [`Mulutiproxy_System_AWS_10min_ver3.pptx`](./Mulutiproxy_System_AWS_10min_ver3.pptx)

> **このポートフォリオの位置づけ**: 実務で扱う構成を題材にした **個人OSS開発（学習・検証目的）** です。
> 商用設定のコピーではなく、構造と責務から自作しています。機密情報は一切含みません。
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>A</td>
          <td>AWS への移行設計</td>
          <td>ローカルで分解した設計をクラウドに再配置し、境界設計を再検証する（<a href="./aws-deployment-plan.html">展開案を整理</a>）</td>
          <td>計画ドキュメント整理済み</td>
        </tr>
        <tr>
          <td>A</td>
