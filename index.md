...
ページが長いため、**閲覧時間に応じた読み方**をご案内します。

| 時間 | 推奨ルート | 何が分かるか |
|---|---|---|
| 3分 | [✅ 動作検証（証跡）](#-動作検証スクリーンショット集) → [Verification](./verification.html) | 設計どおりに動いている証拠（スクショ／ログ） |
| 5分 | [このポートフォリオで伝えたいこと](#このポートフォリオで伝えたいこと) → [2. システムアーキテクチャ](#2-システムアーキテクチャ全体像) | 全体像と設計の軸（責務分離） |
| 10分 | [4. 設計補足](#4-設計補足通信制御に関する考慮点) → [5. 苦労した点](#5-苦労した点と解決アプローチ) | 設計判断の理由と切り分け能力 |
| 追加 | [AWS 展開案](./aws-deployment-plan.html) | クラウド移行設計案（実装未実施） |
| 補足 | [ポートフォリオ説明用スクリプト](./interview-pitch.html) | 口頭説明用のスピーカーノート |

### 📌 発表資料（PPTX）

本ポートフォリオは、Markdown による詳細説明に加えて、短時間で全体像を把握するための発表資料も用意しています。

- **5分概要版** — オンプレ相当検証環境の全体像、代表的なログ切り分け、再現性を確認する資料: [`Mulutiproxy_System_AWS_5min _ver.2.pptx`](./Mulutiproxy_System_AWS_5min%20_ver.2.pptx)
- **10分詳細版** — 3段Proxy、認証・DNS・PAC、ログ・監視、AWS移行設計案まで含めた詳細説明資料: [`Mulutiproxy_System_AWS_10min (1).pptx`](./Mulutiproxy_System_AWS_10min_ver3.pptx)
