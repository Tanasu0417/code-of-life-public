...
- 本ページは「設計の発想・判断軸・段階導入計画」を示し、口頭で説明できる形に整理することを目的としています。
...
- **運用保守経験を AWS 設計に接続しやすい** — オンプレで身につけた切り分け観点を口頭説明と実装の両方で語れる
...
| 10 | 1 枚図 / 10 分説明資料化 | interview-pitch.md / PPTX |
...
*1行サマリ: 口頭説明時に深掘りされそうな技術論点を先回りして整理する。*
...
## 11. 説明ポイント（短時間版 + 詳細版の二段構え）

*1行サマリ: 概要を聞きたい層には「動いている OSS 基盤と AWS 設計力」、技術者には「責務分離と段階導入」を語る。*
...
> 設計判断・段階導入計画・コスト方針・想定論点まで整理しているので、最小構成から IaC 化を進められる状態です。

**短時間で伝わるキーワード**：
- OSS で **実装済み**、動作証跡もある
- AWS は **設計段階**、Phase 1 から段階導入予定
- **コストと運用負荷を意識** した中小規模向け構成
