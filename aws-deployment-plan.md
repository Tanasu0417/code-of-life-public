...
- 現行のオンプレ / Docker Compose構成を AWS 上に再構成する **設計案**
...
- **案B：推奨（バランス型）** — 3段Proxy維持＋AWSマネージド活用で運用とコストを両立
- **案C：監査強化（高セキュリティ）** — Network Firewall / OpenSearch / Security Hub / AWS Config / CloudTrail / S3 を組み合わせ。高コスト・高機能
- **採用：案 B** — 認証は Samba AD/DC、VPN 経由のみ受付、PAC 配布で経路選択、3 段 Proxy で責務分離、AWS サービスを必要なところに留めて運用
- **将来検討**：Managed Microsoft AD への段階移行
...
- **VPN接続** — AWS Client VPN（社内 / 社外の Client が VPC へ到達する唯一の経路）
- **PAC（CloudFront + S3）** — 経路選択を行う設定ファイルを CDN 配布
- **Proxy1 / Proxy2 / Proxy3** — Private Subnet に配置し、責務を分離
  - Proxy1：認証・入口制御
  - Proxy2：経路分岐・ICAP / ClamAV 連携
  - Proxy3：出口制御
- **ICAP / ClamAV** — Proxy2 と連携してコンテンツ検査
- **AD Connector → Samba AD/DC** — AD Connector を AWS 側参照口、認証の正本はオンプレ Samba AD/DC（VPN 経由で参照）
- **CloudWatch Logs / Alarm** — ログ収集・異常検知
- **S3（Log Archive）** — ライフサイクル管理付きの長期保管
- **SSM Session Manager** — 踏み台不要の安全な EC2 アクセス
- **NAT Gateway** — Private Subnet からインターネットへの出口集約
- **将来検討（段階導入）**：OpenSearch Service / AWS Network Firewall / Managed Microsoft AD

### 1-3. 通信フロー図

![通信フロー図 - B案 3段Proxy + PAC制御](./images/03_network_flow.png)

下から上へ通信が進みます。図の番号順に説明します。

1. **Client起動**（Windows PC、GPO 配布済み）
2. **VPN接続**（AWS Client VPN：認証 ID / MFA、ルートテーブルで Proxy1 方向へ中継、VPN 以外の通信は遮断）
3. **PAC（経路選択）**（CloudFront + S3 から `proxy.pac`、DIRECT or Proxy 振分）
4. **Proxy1：認証**（AD Connector → Samba AD/DC を Kerberos / LDAP で参照、入口 ACL）
5. **Proxy2：経路分岐**（中継、ICAP サービスへ転送、内部通信は TLS：stunnel または Squid TLS）
6. **ICAP / ClamAV：検査ポイント**（不正検査・ウイルス検査・ファイルチェック）
7. **Proxy3：出口制御**（経路分岐、拒否ポリシー）
8. **NAT Gateway**（Public Subnet 配置、外部接続の集約点）
9. **Internet** へ HTTP / HTTPS で到達

横断の **運用・監視・ログ基盤**：CloudWatch Logs（収集）→ S3（永続保管）+ SSM Session Manager（踏み台不要）。
**将来検討**：OpenSearch Service / AWS Network Firewall / Managed Microsoft AD。

### 1-4. レイヤー分解図

![アーキテクチャ - 3段Proxy レイヤー分解図](./images/04_architecture_layers.png)

責務を 6 レイヤ + Logging Layer（横断）に分離します。

- **Client Layer（利用者）** — Windows PC、ユーザー端末
- **Access Layer（接続）** — AWS Client VPN（VPN 以外の通信を遮断、認証 ID / MFA、ルートテーブルで Proxy1 へ中継）
- **Control Layer（経路制御）** — PAC（CloudFront + S3）で通信元と経路選択
- **Auth Layer（認証）** — AD Connector が VPN 経由で Samba AD/DC（認証の正本）を参照、Kerberos / LDAP / GPO 配布
- **Proxy Layer（通信制御）** — Proxy1（入口・認証）→ Proxy2（経路分岐・ICAP 連携・TLS 再暗号化）→ Proxy3（出口制御、HTTP / HTTPS 80 / 443）
- **Egress Layer（外部通信）** — NAT Gateway（Public Subnet）で出口を集約 → Internet
- **Logging Layer（運用・横断）** — CloudWatch Logs（ログ収集・認証失敗・アクセスログ）、S3（永続保管・ライフサイクル管理・SSM 操作の証跡）、SSM Session Manager（踏み台不要の安全アクセス）
...
| Samba AD/DC（オンプレ・正本） | **オンプレに正本を残置**、AWS は **AD Connector で参照** | 認証の正本を変えずに段階移行 | DNS / Kerberos / 時刻同期、AD Connector の設置サブネット |
| dnsmasq / WPAD（社内 PAC 配布） | **PAC（CloudFront + S3）** | 静的配信で運用コスト低、CDN で大規模配布も対応 | PAC キャッシュ TTL、HTTPS 配布、社内 DNS との整合 |
| Proxy1（入口・認証） | **EC2 + Squid（Proxy1）** | 既存 ACL / 認証ログ仕様を踏襲 | AD Connector 連携、認証失敗ログ集約 |
| Proxy2（中継・経路分岐） | **EC2 + Squid（Proxy2）** | ICAP 連携・経路分岐の責務を維持 | ICAP 待ち時間、Proxy 間 SG 設計 |
| Proxy3（出口・ACL） | **EC2 + Squid（Proxy3）** | 出口 ACL と NAT GW 集約 | NAT GW のポート確保、宛先 ACL 反映フロー |
| stunnel（中継暗号化） | **内部通信 TLS（stunnel または Squid TLS）** | 図に従い stunnel 相当を維持、ACM / NLB TLS Listener も検討余地 | TLS バージョン、証明書ローテーション運用 |
| ICAP / ClamAV | **EC2 上の ICAP / ClamAV** | 既存検査ロジックを継続利用 | 定義ファイル更新、メモリ使用率、スキャン遅延 |
| Loki / Graylog（ログ検索） | **CloudWatch Logs**（必要時 OpenSearch Service を **将来検討**） | 初期は CW Logs Insights、検索要件が出たら OpenSearch | クエリ性能、ログ保存期間、コスト |
| Zabbix（死活監視） | **CloudWatch Alarm** | マネージドで運用負荷削減、SNS 通知へ接続容易 | アラーム閾値、誤検知率、通知先 |
...
| 可用性 | 低（責務集約で切り分け困難） | 中（2AZ で継続） | 中〜高 |
| 月額コスト目安 | 小 | 中 | 大 |
| 運用負荷 | 高（手動運用中心） | 中 | 中〜高 |
...
- B 案は **既存の責務分離を維持しつつ AWS マネージドを活用**、段階的に C 案へ拡張できる
...
*1行サマリ: 3 段 Proxy + 2AZ + AD Connector + PAC + CloudWatch / S3 / SSM が骨子。*
...
| Proxy EC2 | **3 段（Proxy1 / 2 / 3）** | OSS 構成の責務分離を AWS でも維持 |
| ICAP / ClamAV | EC2 | Proxy2 と連携してコンテンツ検査 |
| 内部通信暗号化 | TLS（stunnel または Squid TLS） | Proxy 間の暗号境界を維持 |
| NAT Gateway | 初期 1 台 / 標準 2 台 | コスト優先 vs 可用性優先のトレードオフ |
| AD Connector | small / large | 既存 Samba AD/DC を VPN 経由で参照 |
| AWS Client VPN | 利用人数で段階拡張 | 社内 / 社外 Client → VPC の唯一の経路 |
| PAC 配布 | CloudFront + S3 | 静的配信で運用コスト低 |
| CloudWatch Logs / Alarm | 必須 | アクセスログ集約・異常検知 |
| S3 Log Archive | 必須 | ライフサイクル管理付きの長期保管 |
| SSM Session Manager | 必須 | 踏み台不要、IAM 監査が効く |
| OpenSearch / Network Firewall / Managed AD | **将来検討（段階導入）** | 監査・検索・ID 基盤刷新の要件発生時に追加 |

---

## 5. 採用構成の設計判断

*1行サマリ: 図にある構成について「なぜそれを選ぶのか」を 6 観点で整理する。*

### 5-1. なぜ 3 段 Proxy なのか

- **Proxy1：認証・入口制御** — AD Connector で Samba AD/DC を参照、入口 ACL、認証失敗の判定
- **Proxy2：経路分岐・ICAP 連携・将来拡張** — 中継、ICAP / ClamAV 連携、TLS 再暗号化、将来の検査機能の追加先
- **Proxy3：出口制御** — 経路分岐、拒否ポリシー、NAT GW への集約点
...
- **VPN 経由でのみ接続** — Client → Proxy への到達は AWS Client VPN を経由
...
- **複数 Proxy への振り分け** — Proxy1 / Proxy2 / Proxy3 や将来の追加 Proxy へ動的分配
- **障害時・検証時の経路切替** — PAC 差し替えのみで経路を切替可能
...
*1行サマリ: CloudWatch Logs を起点に、S3 で長期保管、SSM で安全運用。*
...
- **将来検討**: OpenSearch Service（検索要件発生時）／ AWS Network Firewall（ネットワーク監査強化時）
...
|---|---|---|---|---|---|
| Proxy1 | t3.small または t3.medium | 常時起動 | 認証・入口制御を担当、AD 連携時の CPU / メモリ想定 | CPU / メモリ / 認証失敗数 | レイテンシ悪化、ピーク時のリソース不足 |
| Proxy2 | t3.small または t3.medium | 常時起動 | 経路分岐・ICAP 連携、ICAP 待ちが性能要因 | レイテンシ / ICAP 待ち / メモリ | ICAP 連携の応答遅延、検査スループット不足 |
...
| AD Connector | small / large | 常時起動 | 既存 Samba AD/DC を VPN 経由で参照 | 接続失敗 / Kerberos エラー | ユーザー数増、レイテンシ悪化 |
| AWS Client VPN | 利用人数で段階拡張 | 常時起動 | リモート接続の終端 | 同時接続数 / 認証失敗数 | リモート利用者の増減 |
| OpenSearch Service | 段階導入（将来検討） | - | 監査・検索要件発生時に追加 | 検索レイテンシ / ストレージ使用率 | 検索要件の発生時 |
| Network Firewall | 段階導入（将来検討） | - | 監査強化要件発生時に追加 | ルール件数 / トラフィック量 | ネットワーク監査要件発生時 |
| Managed Microsoft AD | 段階導入（将来検討） | - | ID 基盤を AWS へ集約する場合 | - | ID 基盤刷新時 |
...
- **OpenSearch / Network Firewall / Managed AD は段階導入** — 初期は CW Logs Insights / NACL / AD Connector で代替
...
| 3 | 全体構成図（02）→ 3段Proxy で OSS の責務分離を AWS でも維持 | 1分 | VPC / 2AZ / Proxy1/2/3 / ICAP / NAT / CW / AD Connector / PAC を一枚図で説明 |
| 4 | レイヤー分解図（04）→ Client / Access / Control / Auth / Proxy / Egress / Logging | 45秒 | レイヤ別責務分離と Logging Layer の横断観測 |
| 5 | 設計判断（5-1〜5-6）から 1〜2 点を深掘り | 1分15秒 | 例：「なぜ 3 段を維持するのか」「なぜ AD Connector を選ぶのか」 |
